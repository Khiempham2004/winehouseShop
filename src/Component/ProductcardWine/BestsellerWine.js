...
export default function BestsellerWine() {
  const [bestSeller, setBestSeller] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchBestseller = async (res, req) => {
      try {
  ...
  const handleAddBestseller = (bestSeller) => {
    console.log("Da them vao gio hang : ", bestSeller);
    alert(`${bestSeller.name} da duoc them vao gio hang!`);
  };
  ...
  return (
    <div>
            ...
            {productBestseller.map((item) => (
              ...
              >
                ...
                <div className="text-center p-3">
                  ...
                  <button
                    onClick={() => handleAddBestseller(item)}
      ...
      </div><h2>Bestseller Wine</h2>
      <p>Explore our most popular wines, loved by customers worldwide.</p>
    </div>
  )
}
